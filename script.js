...
let audioElement=new Audio("./goat-official-music-video.mp3")
...
let songNames=[
"Beautiful","When I'm Gone","Mockingbird","Stan ft.Dido","Ammaye Sannaga","Yu hi re","Sariyaagi","Nadaan Parindey","Sadda Haq","Tu hai ki nahi","Ghal Ghal","Huttidare"
]
...
searchButton.addEventListener('click', (e) => {
    ...
    const songs = document.querySelectorAll('div');

    songs.forEach((songs, index) => {
        const songNames = songs.textContent.toLowerCase();

        if (songNames.includes(searchTerm)) {
            songs.scrollIntoView({ behavior: 'smooth' });
        }
    });
});